...
class ToolLoopPlanner:
    ...
    def _build_tools(self) -> Dict[str, Callable[..., Awaitable[Any]]]:
        ...
        async def observe_tool(query: str, response_format_str: str = "str") -> str:
            ...
            result = await self._controller.observe(
                query, response_format=actual_response_format
            )
    ...
    def _fresh_llm_client(self) -> AsyncUnify:
        """Creates a new AsyncUnify client."""
        client = AsyncUnify(
            os.environ.get("UNIFY_MODEL_GPT_4O", "gpt-4o@openai"),
        ...
        async def _finalizer():
            try:
                ...
                if self._loop_handle is handle:
                    ...
                    self._client = None
                    # self._task_id could be cleared here if truly done, or kept for post-mortem.
                    # For now, let's clear it if moving to IDLE.
    ...
    async def execute(self, task_description: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Planner: Task {self._task_id} execution failed: {e}", exc_info=True
            )
            self._set_state(_PlannerState.IDLE)
            self._task_id = None
    ...
    async def resume(self) -> None:
        ...
        self._client = self._fresh_llm_client()
        # Note: parent_chat_context is passed to start_async_tool_use_loop,
        # which handles appending it to the client's message history.
    ...
    async def query(self, question: str) -> str:
        ...
        async def _wait_for_reply() -> str:
            ...
            while asyncio.get_event_loop().time() < end_time:
                ...
                for i in range(
                    start_idx + 1, len(current_messages)
    ...
    async def __aenter__(self) -> "ToolLoopPlanner":
        logger.info("ToolLoopPlanner entering context.")
        # Placeholder for any resource setup if needed in the future
        return self

    async def __aexit__(self, exc_type, exc, tb) -> None:
        logger.info(
            "ToolLoopPlanner exiting context. Attempting to stop any active/paused task."
        )
        await self.stop()  # Ensure any running task is stopped

        # Clean up clarification queues to prevent listeners from hanging
        while not self._clar_up_q.empty():
            try:
                self._clar_up_q.get_nowait()
            except asyncio.QueueEmpty:
                break
        while not self._clar_down_q.empty():
            try:
                self._clar_down_q.get_nowait()
            except asyncio.QueueEmpty:
                break
        logger.info("ToolLoopPlanner context exited.")
