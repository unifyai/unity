...
class ConversationManagerState:
    ...
    def update_state(self, event: Event):
        ...
        match event:
            case PhoneCallRecieved() as e:
                ...
                self.push_message(contact, "phone", Message(contact.full_name, "<Phone call sent...>", e.timestamp))
                self.push_notif(Notification("comms", f"Phone Call recieved from '{contact.full_name}'", e.timestamp))
            
            # made by assistant 
            case PhoneCallSent() as e:
                contact = self.get_contact(phone_number=e.contact)
                self.push_message(contact, "phone", Message("You", "<Phone call sent...>", e.timestamp))
                self.push_notif(Notification("comms", f"Phone Call Sent to '{contact.full_name}'", e.timestamp))
            case PhoneCallStarted() as e:
                self.mode = "call"
                contact = self.get_contact(phone_number=e.contact)
                self.phone_contact = contact
                self.push_message(contact, "phone", Message(contact.full_name, "<Phone call started...>"))
                self.push_notif(Notification("comms", f"Phone Call started with '{contact.full_name}'", e.timestamp))
        
            case PhoneUtterance() as e:
                contact = self.get_contact(phone_number=e.contact)
                self.push_message(contact, "phone", Message(contact.full_name, e.content, e.timestamp))
                self.push_notif(Notification("comms", f"Phone Call Utterance recieved from '{contact.full_name}'", e.timestamp))
            # made by assistant
            case AssistantPhoneUtterance() as e:
                contact = self.get_contact(phone_number=e.contact)
                self.push_message(contact, "phone", Message("You", e.content, e.timestamp))
                self.push_notif(Notification("comms", f"Phone Call Utterance sent to '{contact.full_name}'",e.timestamp))
            case SMSRecieved() as e:
                contact = self.get_contact(phone_number=e.contact)
                self.push_message(contact, "sms", Message(contact.full_name, e.content, e.timestamp))
                self.push_notif(Notification("comms", f"SMS recieved from '{contact.full_name}'",e.timestamp))
            # made by assistant
            case SMSSent() as e:
                contact = self.get_contact(phone_number=e.contact)
                self.push_message(contact, "sms", Message("You", e.content, e.timestamp))
                self.push_notif(Notification("comms", f"SMS sent to '{contact.full_name}'", e.timestamp))
            case EmailRecieved() as e:
                contact = self.get_contact(email=e.contact)
                self.push_message(contact, "email", EmailMessage(contact.full_name, e.subject, e.body, e.timestamp))
                self.push_notif(Notification("comms", f"Email recieved from '{contact.full_name}'", e.timestamp))
            # made by assistant
            case EmailSent() as e:
                contact = self.get_contact(email=e.contact)
                self.push_message(contact, "email", EmailMessage("You", e.subject, e.body, e.timestamp))
                self.push_notif(Notification("comms", f"Email sent to '{contact.full_name}'", e.timestamp))
            # made by assistant
            case Error() as e:
    ...
    def get_state_for_llm(self) -> list[dict]:
        active_convs = "\n\n".join(self._render_contact(c) for c in self.active_conversations.values())
        notif = self._render_notifs()
        state = f"<notifications>\n{self._add_spaces(notif)}\n</notifications>\n<active_conversations>\n{self._add_spaces(active_convs)}\n</active_conversations>"

        return state
    
    def push_message(self, contact: Contact, thread: Literal["sms", "email", "phone"], message: Message | EmailMessage):
    ...
    # contacts helpers
    def get_contact(self, id: Optional[str] = None, phone_number: Optional[str] = None, email: Optional[str] = None):
        """returns the new contact and whether they were newly created or not."""
        if not (id or phone_number or email):
            raise Exception("at least one id, phone number or email must be provided to infer contact")
        if id:
    ...
    def update_or_create_new_contact(self, id: str, first_name: str, last_name: Optional[str] = None, email: Optional[str] = None, phone_number: Optional[str] = None):
        contact = None
        if id != "-1": # update branch
            ...
            new_contact = Contact(str(len(self.phone_contacts_map) + 1), first_name, last_name, email, phone_number)
    ...
    def _render_notifs(self):
        return "\n".join(
            [
                ...
                for n in self.notifs if n.timestamp > self.last_snapshot_time
            ])
