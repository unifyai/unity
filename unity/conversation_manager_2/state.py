...
from .prompt_utils import ThreadMessage, ConversationContact, ContactThread, NotificationBar, add_spaces
...
class ConversationManagerState:
    ...
    def push_event(self, event: Event):
        contact = self.phone_contacts_map.get(event.contact) or self.email_contacts_map.get(event.contact)
        if not contact:
        ...
        if isinstance(event, PhoneCallInitiated):
            active_c.push_message("phone", message=ThreadMessage(contact.name, "<Phone call Initiated...>", event.timestamp))
            self.notifications.push_notif(f"Phone Call Initiated by '{contact.name}'", event.timestamp)
        elif isinstance(event, PhoneCallStarted):
            active_c.push_message("phone", message=ThreadMessage(contact.name, "<Phone call Started...>", event.timestamp))
            self.notifications.push_notif(f"Phone Call Started with '{contact.name}'", event.timestamp)
        elif isinstance(event, PhoneUtterance):
            active_c.push_message("phone", message=ThreadMessage(contact.name, event.content, event.timestamp))
            self.notifications.push_notif(f"Phone utterance recieved from '{contact.name}'", event.timestamp)
        elif isinstance(event, PhoneCallEnded):
            active_c.push_message("phone", message=ThreadMessage(contact.name, "<Phone call Ended...>", event.timestamp))
            self.notifications.push_notif(f"Phone Call Ended with '{contact.name}'", event.timestamp)



        elif isinstance(event, SMSRecieved):
            active_c.push_message("sms", message=ThreadMessage(contact.name, event.content, event.timestamp))
            self.notifications.push_notif(f"SMS recieved recieved from '{contact.name}'", event.timestamp)
        elif isinstance(event, EmailRecieved):
            ...
        # assistant events
        elif isinstance(event, SMSSent):
            active_c.push_message("sms", message=ThreadMessage("You", event.content, event.timestamp))
            self.notifications.push_notif(f"SMS sent to '{contact.name}'", event.timestamp)

        if contact_added:
            self.notifications.push_notif(f"Added contact '{contact.name}' to active conversations", event.timestamp)

    
    def add_contact_to_active_conversations(self, contact, on_phone=False, timestamp=None):
        if contact.id in self.active_conversations:
            return
        self.active_conversations[contact.id] = ConversationContact(contact.id, contact.name, contact.is_boss, on_phone=on_phone)
