...
def build_brain_spec(
    ...
    # Resolve the boss contact via the bootstrap-delivered ``boss_contact_id``;
    # ContactManager is the source of truth for the full row.
    boss_contact_id = SESSION_DETAILS.user.contact_id
    boss_contact = (
        cm.contact_index.get_contact(int(boss_contact_id)) or {}
    ...
    system_prompt = build_system_prompt(
        bio="\n".join(_bio_parts),
        contact_id=int(boss_contact_id) if boss_contact_id is not None else None,
