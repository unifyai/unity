...
def get_broader_context(
    ...
    1. Fetch the assistant's self-contact and the boss (user) contact from
       the body's Contacts table using the bootstrap-resolved
       ``SESSION_DETAILS.assistant.contact_id`` and
       ``SESSION_DETAILS.user.contact_id`` via a *lazy* ContactManager import
       so we do **not** introduce hard import cycles during interpreter
       start-up.
    ...
    with _LOCK:
        ...
        from unity.session_details import SESSION_DETAILS  # lazy; avoid cycle
        ...
        if self_contact_id is None or boss_contact_id is None:
            raise RuntimeError(
                "broader_context requires resolved self_contact_id/boss_contact_id; "
                "bootstrap has not yet materialized the body's ContactMembership overlays.",
            )

        assist_rows = cm.filter_contacts(
            filter=f"contact_id == {int(self_contact_id)}",
            limit=1,
        ).get("contacts", [])
        user_rows = cm.filter_contacts(
            filter=f"contact_id == {int(boss_contact_id)}",
            limit=1,
        ).get("contacts", [])

        assist = assist_rows[0]
        u = user_rows[0]
