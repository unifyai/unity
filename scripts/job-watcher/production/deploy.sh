...
# Deploy the job-watcher to the production namespace on the unity GKE cluster.
#
# Prerequisites:
...
#   - unity-config configmap and unity-secrets secret exist in production namespace
...
echo "=== Deploying job-watcher to production ==="
kubectl apply -f "$SCRIPT_DIR/deployment.yaml"

echo ""
echo "Waiting for pod to be ready..."
kubectl rollout status deployment/job-watcher -n production --timeout=120s

echo ""
echo "=== Deployment complete ==="
echo ""
echo "Check logs with:"
echo "  kubectl logs -n production -l app=job-watcher -f"
