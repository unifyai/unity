...
# Deploy Grafana to the production namespace on the unity GKE cluster.
#
# Prerequisites:
...
#   - comm-sa-key secret already exists in production namespace
#
# Usage:
...
DOMAIN="grafana.internal.saas.unify.ai"
...
echo "[1/5] Creating grafana-secrets..."
...
while [ -z "$EXTERNAL_IP" ] && [ $ATTEMPTS -lt $MAX_ATTEMPTS ]; do
  EXTERNAL_IP=$(kubectl get ingress grafana -n "$NAMESPACE" -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || true)
...
if [ -z "$EXTERNAL_IP" ]; then
  ...
  echo "  kubectl get ingress -n $NAMESPACE grafana --watch"
  echo ""
...
echo ""
echo "Access Grafana at: https://$DOMAIN"
echo "  Username: admin"
echo "  Password: $ADMIN_PASSWORD"
echo ""
echo "After the script finishes, apply the ManagedCertificate for HTTPS:"
echo "  kubectl apply -f $SCRIPT_DIR/managed-certificate.yaml"
echo "  kubectl apply -f $SCRIPT_DIR/ingress.yaml"
echo ""
echo "Monitor certificate provisioning (10-15 minutes):"
echo "  kubectl get managedcertificate grafana-cert -n $NAMESPACE --watch"
