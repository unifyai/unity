                    ...
                    "spec": {
                        ...
                        "containers": [{
                            ...
                            "env": [
                                ...
                                {"name": "ASSISTANT_NUMBER", "value": assistant_number},
                                {"name": "USER_NUMBER", "value": user_number},
                                {"name": "USER_PHONE_NUMBER", "value": user_phone_number or user_number},
                            ...
                            "volumeMounts": [
                                {"name": "sa-key", "mountPath": "/secrets", "readOnly": True}
                            ]
                        }],
                        "volumes": [
...
def get_job_status(batch_api, core_api, assistant_id: str, namespace: str = "default"):
    ...
    try:
        ...
        status = {
            ...
            "namespace": namespace,
            "creation_timestamp": api_response.metadata.creation_timestamp.isoformat() if api_response.metadata.creation_timestamp else None,
        ...
        for pod in pods.items:
            pod_status = {
                ...
                "scheduled": pod.status.conditions is not None and any(
...
def cleanup_completed_jobs(batch_api, namespace: str = "default", max_age_hours: int = 24, force_all: bool = False):
    ...
    try:
        ...
        for job in jobs.items:
            ...
            if (job.status.succeeded or job.status.failed) and job.metadata.creation_timestamp:
...
Examples:
  ...
  python pubsub_trigger.py --assistant-id test --user-name "John" --user-number "+1234567890"
            ...
            if status['pods']:
                ...
                for pod in status['pods']:
                    ...
                    if 'scheduling_issue' in pod:
                        ...
                        print(f"       ❌ Scheduling issue: {issue['reason']} - {issue['message']}")
            else:
    ...
    if job:
        ...
        print("\n💡 Next steps:")
        print(f"   1. Check job status: python create_job.py --assistant-id {args.assistant_id} --status")
        print(f"   2. View pod logs: kubectl logs -n {args.namespace} -l job-name={job.metadata.name}")
        print(f"   3. Delete job: python create_job.py --assistant-id {args.assistant_id} --delete")
    else:
