    ...
    try:
        ...
        for secret_name in required_secrets:
            try:
                ...
                secrets_data[secret_name] = base64.b64encode(secret_value.encode()).decode()
                print(f"   ✅ {secret_name}")
        ...
        try:
            ...
            if e.status == 404:
                api_client.create_namespaced_secret(namespace=namespace, body=secret_manifest)
        ...
        try:
            ...
            sa_key_manifest = {
                ...
                "type": "Opaque",
                "data": {
                    "key.json": base64.b64encode(key_data).decode()
                }
            }
            
            try:
                api_client.read_namespaced_secret(name="comm-sa-key", namespace=namespace)
                print("✅ Service account key secret already exists")
            except ApiException as e:
                if e.status == 404:
                    api_client.create_namespaced_secret(namespace=namespace, body=sa_key_manifest)
...
Examples:
  ...
  python setup_k8s_config.py --update              # Update existing resources
    ...
    if args.delete:
        confirm = input("⚠️  Are you sure you want to delete all Unity resources? (y/N): ")
        if confirm.lower() == 'y':
    ...
    if args.create or args.update:
        ...
        print("   2. Create a test job: python create_job.py --assistant-id test --user-name 'Test' --user-number '+1234567890'")
