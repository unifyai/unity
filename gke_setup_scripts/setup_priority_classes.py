        ...
        try:
            ...
            try:
                ...
                if e.status == 404:
                    ...
                    print(f"✅ Created PriorityClass '{pc['name']}' (value: {pc['value']})")
                    created_count += 1
                else:
...
def delete_priority_classes(api_client):
    ...
    unity_priority_classes = ["unity-critical", "unity-high", "unity-normal", "unity-low"]
    deleted_count = 0
...
Usage in Jobs:
  spec:
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="List all priority classes"
    )
