...
@pytest.mark.asyncio
@_handle_project
async def test_real_conductor_task_ask_calls_task_scheduler():
...
@pytest.mark.asyncio
@_handle_project
async def test_real_conductor_task_update_calls_task_scheduler():
    actor = SimulatedActor(steps=0)
    ts = TaskScheduler(actor=actor)
    cond = SimulatedConductor(task_scheduler=ts, actor=actor)
    ...
    assert tasks and any(
        (t.name or "").lower() == "promote jeff smith"
...
@pytest.mark.asyncio
@_handle_project
async def test_real_conductor_task_execute_calls_task_scheduler():
