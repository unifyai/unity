...
async def test_highlight_text_with_drag(backend):
    ...
    instr = "Drag over the text 'A TEST' to highlight it."
    await backend.act(
        instr,
    )
...
async def test_scroll_until_ready(backend):
    ...
    await backend.act(
        "Type 'for i in $(seq 1 70); do echo LINE $i; done' and press Enter.",
    )
...
async def test_click_positions(backend):
    ...
    x1, y1 = 200, 300
    x2, y2 = 300, 400
    instr1 = f"Click at pixel ({x1},{y1}). Then click at pixel ({x2},{y2})."
    await backend.act(
        instr1,
    )
