...
@_handle_project
@pytest.mark.asyncio
async def test_race_condition_silent_flush_vs_speech_event(mocked_screen_share_manager):
    """
    Ensures _pending_vision_events are processed exactly once when a silent flush
    and a speech event are triggered concurrently.
    """
    manager, mocks = mocked_screen_share_manager
    manager.DEBOUNCE_DELAY_SEC = 0.05
    manager.INACTIVITY_TIMEOUT_SEC = 0.1

    # Use asyncio Events to control the execution flow of a mocked _analyze_turn
    analysis_started = asyncio.Event()
    proceed_analysis = asyncio.Event()

    original_analyze_turn = manager._analyze_turn

    async def controlled_analyze_turn(*args, **kwargs):
        analysis_started.set()  # Signal that analysis has begun
        await proceed_analysis.wait()  # Wait for the test to allow continuation
        return await original_analyze_turn(*args, **kwargs)

    with patch.object(
        manager, "_analyze_turn", side_effect=controlled_analyze_turn, autospec=True
    ) as mock_analyze_turn:
        # 1. Setup initial state: one pending vision event
        manager._pending_vision_events.append(
            {"timestamp": 1.0, "before_frame_b64": "b", "after_frame_b64": "a"}
        )
        # Set last activity time far in the past to make the timeout flush eligible
        manager._last_activity_time = asyncio.get_event_loop().time() - 1.0

        # 2. Start the silent flush. It will call _analyze_turn and then pause.
        flush_task = asyncio.create_task(manager._flush_pending_events_on_timeout())
        await asyncio.wait_for(analysis_started.wait(), timeout=1)

        # 3. At this point, the silent flush is "stuck" inside _analyze_turn.
        # Now, trigger a competing speech event.
        speech_event = {
            "payload": {"content": "test", "contact_details": {"contact_id": 1}}
        }
        manager._trigger_turn_analysis(speech_event)

        # 4. Allow all tasks to proceed
        await asyncio.sleep(manager.DEBOUNCE_DELAY_SEC + 0.01)
        proceed_analysis.set()
        await asyncio.sleep(0.1)  # Allow all background tasks to complete

        # 5. Assertions
        # The debounce logic should ensure only ONE analysis is ultimately run.
        # The critical part is which one and what data it gets.
        # The speech event should cancel the silent flush's debounced task.
        mock_analyze_turn.assert_called_once()

        # Check the arguments of the single call to _analyze_turn
        call_args, _ = mock_analyze_turn.call_args
        # Arg 1 is self, Arg 2 is speech_event, Arg 3 is visual_events
        passed_speech_event = call_args[1]
        passed_visual_events = call_args[2]

        # It should have been called with the speech event
        assert passed_speech_event == speech_event
        # And it should have consumed the pending visual events
        assert len(passed_visual_events) == 1
        assert passed_visual_events[0]["timestamp"] == 1.0

        # The pending events list should now be empty.
        assert len(manager._pending_vision_events) == 0
...
@_handle_project
@pytest.mark.asyncio
async def test_race_condition_rapid_debounce_cancellation(mocked_screen_share_manager):
    """
    Verifies that rapid-fire speech events correctly cancel previous debounced
    tasks and only the final event triggers an analysis.
    """
    manager, mocks = mocked_screen_share_manager
    manager.DEBOUNCE_DELAY_SEC = 0.1

    # Spy on the real _analyze_turn to check its arguments
    with patch.object(manager, "_analyze_turn", autospec=True) as mock_analyze_turn:
        # Trigger three speech events in quick succession, faster than the debounce delay
        speech_event_1 = {
            "payload": {"content": "first", "contact_details": {"contact_id": 1}}
        }
        speech_event_2 = {
            "payload": {"content": "second", "contact_details": {"contact_id": 1}}
        }
        speech_event_3 = {
            "payload": {"content": "third", "contact_details": {"contact_id": 1}}
        }

        manager._trigger_turn_analysis(speech_event_1)
        await asyncio.sleep(0.02)
        manager._trigger_turn_analysis(speech_event_2)
        await asyncio.sleep(0.02)
        manager._trigger_turn_analysis(speech_event_3)

        # Wait for the final debounce window to elapse
        await asyncio.sleep(manager.DEBOUNCE_DELAY_SEC + 0.05)

        # Assert that analysis was only called once
        mock_analyze_turn.assert_called_once()

        # Assert that the analysis was called with the data from the LAST event
        call_args, _ = mock_analyze_turn.call_args
        passed_speech_event = call_args[1]
        assert passed_speech_event == speech_event_3
